Dear Board Members,

Thank you for attending the Learn and Help Board Meeting held on **Wednesday, June 24, 2026**, via Zoom.

Please review the draft meeting minutes below and share any corrections, additions, or feedback. If there are no changes, please reply with your approval so we can finalize these minutes for our records.

---

# Learn and Help Board Meeting Minutes

**Date:** Wednesday, June 24, 2026
**Location:** Zoom
**Purpose:** Board Meeting

## 1. 2025–2026 Year-End Summary

### Students Served

* Python 101: **30 students**
* Python for Data Science: **11 students**
* Python for Machine Learning: **8 students**

### Financial Summary

**Revenue**

* Registration fees collected: **$26,750**

**Community Support**

* 128 regular school libraries @ $100 each
* 256 summer school libraries @ $50 each

**Total donated:** **$25,600**

**Expenses**

* Manabadi Dallas Event: **$300**
* Banking/Checking expenses: **$40**

**Current Bank Balance:** **$810**

Outstanding obligations:

* Six nonprofit-sponsored student reimbursements
* Two late mentorship reimbursements

**Amount owed:** **$800**

**Projected closing balance:** **$10**

The Board appreciated the successful completion of another year in which virtually all program proceeds were returned to the community.

---

# 2. Summer 2026 Program

### Spark Civics: Think, Build, and Pitch

* **Instructor:** Samhita Krishnamurthy
* **Schedule:** Mondays and Wednesdays
* **Time:** 7:30 PM – 9:00 PM CST
* **Dates:** July 6 – August 26, 2026
* **Fee:** $256
* **Enrollment:** 8 students registered. 

---

# 3. 2026–2027 Academic Year Planning

### Proposed Courses

* Python 101 — $600
* Python for Data Science — $600
* Code for Good — $600
* Upskill (Adults) — $1,000

### Code for Good Project Options

Students may:

* Build a website for a nonprofit organization
* Start their own nonprofit and develop its website
* Contribute to an existing nonprofit (such as Project ABCD, TeluguPuzzles, IndicPulse, etc.)
* Build a website for a startup or small business

### Proposed Schedule

* Sundays
* September 13, 2026 – May 30, 2027

### Discussion: End-of-Year Scheduling

The Board discussed conflicts caused by school activities during the final weeks of May.

Possible approaches included:

* Publish final project demonstration dates very early (possibly by Thanksgiving)
* Encourage students to communicate conflicts with instructors as early as possible
* Allow one scheduled makeup opportunity when necessary

Decision: We will take up a combination of above approaches to reduce the contention. 

---

# 4. Registration Incentives and Policies

The Board discussed several strategies to encourage early registration.

### Proposed Registration Timeline

**Register by July 31**

* $600 tuition
* Registration fee waived

**Beginning August 1**

* August 31 Open House
* Open Hour will also serve as Trial class

* Tuition: $600
* Registration fee: $50

**Beginning 14th September (after classes begin)**

* $600 tuition
* $50 registration fee
* $50 late registration fee


**Action:** Board approval needed.

# 5. Referral Program

Students referring another enrolled student will receive a **$50 tuition credit** per successful referral.
The referral program is valid only for Python 101 registrations.  There is no limit on the number of referrals. $50 tuition credit will be given per successful referral.

**Action:** Board approval needed.

---

# 6. Refund Policy

This Refund Policy is applicable to any student joining "Learn and Help" classes for the first time. For example, this refund policy is valid and applicale to the students joining - Python 101 and SparkCivics.

Students withdrawing on or before the second class receive full refund less **$50 registration fee**.

**Action:** Board approval needed.

---

# 7. Budget Request (2026–2027)

The following annual operating budget was presented for approval.

| Item                       | Amount |
| -------------------------- | -----: |
| Website and Hosting        |   $260 |
| OpenRouter AI Subscription |   $240 |
| Marketing and Outreach     | $1,000 |

Marketing budget includes participation in:

* IAM (MN) events
* Manabadi events
* TEAM events
* Social media outreach

**Total requested budget:** **$1,500**

**Action:** The Board was requested to review and approve the proposed budget.

---

# 8. Expanding Community Giving

The Board discussed expanding Learn and Help's charitable impact beyond school libraries.

### Proposed "Pick Your Own Cause" Program

Examples discussed:

* Camp Masala
* Language schools and nonprofit educational organizations such as Manabadi, Pathashala, SILC, Bal Vihar, etc.

Proposed guidelines:

* Organization must be a registered nonprofit
* Organization should align with Learn and Help's mission
* Program initially limited to Python 101 registrations
* Optional premium tuition ($1,000) for families selecting this giving option

Further policy development is required before implementation.

---

# 9. Outreach and Marketing

Marketing activities for July and August were discussed.

### Proposed Leads

* Lalitha — Manabadi outreach
* Brinda — IAM events
* Jasthi — Indian community associations (Telugu, Tamil, Kannada, and other organizations)

---

# 10. Matching Grants and Fundraising

The Board agreed that matching gifts represent an important growth opportunity.

Topics discussed included:

* Corporate matching programs
* Scholarship sponsorships
* Registration on fundraising platforms such as Benevity, Double the Donation, and YourCause

**Action:** Uma and Varma will investigate fundraising platform registration and report back.

---

# 11. Learn and Help Digital Assets

The Board discussed continued improvements to:

* Website
* GitHub repositories
* Facebook
* WhatsApp
* LinkedIn

---

# 13. Instructor Evaluation Form

Jasthi reported that the online instructor evaluation form is still under development and will be integrated directly into the Learn and Help website.

**Action:** Jasthi to complete implementation.

---

# 13. Other Business

No additional agenda items were raised.

---

## Action Items Summary

### 🗳️ Board Approval Required

| # | Action Item | Owner | Status |
|---|-------------|-------|--------|
| 4 | Registration Incentives and Policies | Board | ☐ Pending Approval |
| 5 | Referral Program | Board | ☐ Pending Approval |
| 6 | Refund Policy | Board | ☐ Pending Approval |
| 7 | Budget Request (2026–27) | Board | ☐ Pending Approval |

---

### 📌 Other Action Items

| # | Action Item | Owner | Status |
|---|-------------|-------|--------|
| 9.1 | Manabadi Outreach and Marketing | Lalitha | ☐ In Progress |
| 9.2 | IAM (Minnesota) Outreach and Marketing | Brinda | ☐ In Progress |
| 9.3 | Indian Community Associations Outreach and Marketing | Dr. Siva Jasthi | ☐ In Progress |
| 10 | Investigate matching grants and fundraising platforms; evaluate registration options and report findings | Uma & Varma | ☐ In Progress |
| 13 | Integrate the Instruction Evaluation Form into the Learn and Help website | Dr. Siva Jasthi | ☐ In Progress |

---

Please review these draft minutes and provide any comments, corrections, or suggested edits. If you agree with the contents, kindly reply with your approval so we can finalize the meeting minutes.

Thank you for your continued support and guidance.

Warm regards,

Siva Jasthi
President
Learn and Help
Empowering Minds, Inspiring Generosity!
www.learnandhelp.com
